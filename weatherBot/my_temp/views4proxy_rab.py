...
proxy_https = {'https': 'https://ua-139-170-1.fri-gate0.biz:443/'}
...
def get_updates():
    url = URL + 'getUpdates'
    r = requests.get(url, proxies=proxy_https)
    write_json(r.json())
    return r.json()
...
def send_message(chat_id, text='--Привет, привет!-- )'):
    ...
    r = ProxyRequests(url)
    r = r.post(answer)
    # r = requests.post(url, json=answer, proxies=proxy_https)
...
# https://api.telegram.org/bot919974881:AAHwfCsrATbNx9fxjhbSxzacw5Ip-G-aTKE/setWebhook?url=https://8bdaad6b.ngrok.io/weatherBot/
